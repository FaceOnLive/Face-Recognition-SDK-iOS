            ...
            if(faceBox.liveness > livenessThreshold) {
                ...
                if(maxSimilarity > matchingThreshold) {
                    ...
                    DispatchQueue.main.sync {
                        ...
                        similarityScoreLbl.text = String(format: "%.00f", maxSimilarity*100) + "%"
                        ...
                        self.livenessLbl.text = String(format: "%.03f", faceBox.liveness)
                        self.yawLbl.text = String(format: "%.03f", faceBox.yaw)
                        self.rollLbl.text = String(format: "%.03f", faceBox.roll)
                        self.pitchLbl.text = String(format: "%.03f", faceBox.pitch)
